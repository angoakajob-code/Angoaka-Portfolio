...
const ProjectsSection = () => {
  ...
  const handleToggleShowAll = () => {
    if (showAll) {
      // Réduire : scroll doux vers le haut de la grille avant de masquer
  ...
  // Variants pour les cartes projet
  const cardVariants = {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          <h2 className="text-xl md:text-xl lg:text-2xl font-bold text-white mb-4 relative">
            ...
            <div className="absolute -bottom-9 left-1/2 transform -translate-x-1/2 w-32 h-6 opacity-80">
              <svg width="100%" height="100%" viewBox="0 0 128 24" preserveAspectRatio="none">
                <path d="M0,12 C25,2 50,20 75,8 C100,-4 115,18 128,12" fill="none" stroke="url(#nike-gradient)" strokeWidth="3" strokeLinecap="round"/>
                <path d="M0,12 C25,2 50,20 75,8 C100,-4 115,18 128,12" fill="none" stroke="url(#nike-shadow)" strokeWidth="6" strokeLinecap="round" opacity="0.4" className="blur-[2px]"/>
                <defs>
          ...
          <AnimatePresence>
            {updatedCategories.map((category, index) => (
              <motion.button
                ...
                onClick={() => { setActiveFilter(category.id); setShowAll(false); }}
        ...
        {/* Grid des projets avec AnimatePresence pour entrée/sortie fluide */}
          ...
          <AnimatePresence mode="popLayout">
            {displayedProjects.map((project, index) => (
              <motion.div
                ...
                className="rounded-xl bg-[#2C2C2C] overflow-hidden border border-gray-700/50 hover:border-red-500/50 transition-colors duration-300 cursor-pointer group"
                ...
                <div className="p-5">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.skills.slice(0, 3).map((skill, i) => (
                      <span key={i} className="px-2 py-1 text-gray-50 text-xs rounded">{skill}</span>
                    ))}
                  </div>
                </div>
              </motion.div>
        ...
        {filteredProjects.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-50 text-lg">No projects found in this category.</p>
          </div>
        )}

        {/* Bouton View More / Show Less */}
        {filteredProjects.length > 4 && (
          <div className="text-center">
            <motion.button
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.97 }}
              className="px-8 py-3.5 border-2 border-[#FF0218] bg-transparent text-white rounded-xl font-semibold hover:bg-red-800/20 transition-all duration-300 flex items-center gap-2 mx-auto backdrop-blur-sm"
            >
          ...
          <div className="relative">
            <div className="absolute left-1/2 -translate-x-1/2 -top-6 sm:-top-8 md:-top-10 w-13 h-13 sm:w-14 sm:h-14 md:w-16 md:h-16 lg:w-20 lg:h-20 z-10">
              <div className="relative w-full h-full">
                ...
                <img src={SubtractIcon} alt="V Icon" className="absolute inset-0 w-full h-full p-4 object-contain"/>
              </div>
            </div>
            <div className="bg-gray-100 rounded-2xl w-full max-w-275 mx-auto shadow-xl px-4 sm:px-6 md:px-10 lg:px-16 py-6 sm:py-8 md:py-10">
              <p className="text-white text-sm sm:text-base md:text-lg lg:text-xl leading-relaxed text-center max-w-4xl mx-auto">
